...
/*  Traditionally argument sizes were "int", ie "signed int", even though
 *  negative values didn't make sense.  GNU libc 2 (aka Linux libc6)
 *  changed these argument sizes to be "socklen_t", ie unsigned int, at least
 *  as of GNU libc 2.0.7 included in Debian Linux 2.0 and Red Hat Linux 5.1
 *
 *  If __GLIBC__ is defined, and is 2 or higher, then we use socklen_t here
 *  otherwise we use int.
 */
