/*  ----------------------------------------------------------------<Prolog>-
    ...
    Revised:    98/06/01  iMatix SFL project team <sfl@imatix.com>
  ...
  for(i = 0; i < no; i++) 
    {
      ...
      if (search_patterns[i].pos >= 0)
          sprintf(buffer, "%d", search_patterns[i].pos);
      else
          sprintf(buffer, "ABSENT");
      ...
      if (found != NULL)
          sprintf(buffer, "%ld", (long) (found - search_patterns[i].string));
      else
          sprintf(buffer, "ABSENT");
